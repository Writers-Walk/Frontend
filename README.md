...
| 구분 | 현상 | 원인 | 해결 |
|------|------|------|------|
| 메인 페이지 | 최신순 정렬이 정확하지 않음 | `toLocaleString()` 기반 문자열 날짜 사용 | 날짜 데이터를 ISO 형식으로 통일하여 정확한 시간 비교 및 정렬 구현 |
| 도서 상세 페이지 | 실수로 삭제 버튼 클릭 시 즉시 삭제됨 | 삭제 전 사용자 확인 절차 부재 | `window.confirm()` 확인 창 추가 및 삭제 후 메인 페이지로 이동 |
| 도서 등록 페이지 | 제목 입력창만 다른 UI 스타일 적용 | 브라우저 기본 스타일이 적용됨 | 공통 폰트 및 스타일 속성을 적용하여 UI 통일 |
| 이미지 생성 페이지 | Base64 이미지 저장으로 데이터 용량 증가 및 응답 지연 발생 | 고해상도 이미지 데이터를 그대로 저장 | Canvas API 기반 이미지 압축 기능 구현으로 용량 최적화 |
| 공통 레이아웃 | 모든 페이지에 공통 헤더 적용 어려움 | 기존 라우팅 구조에서 공통 컴포넌트 관리 한계 | Layout + Outlet 기반 중첩 라우팅 구조로 개선 |
---
